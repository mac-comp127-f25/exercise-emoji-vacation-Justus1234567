...
public class EmojiVacation {
    ...
    private static void generateVacationPhoto(CanvasWindow canvas) {
        ...
        // Adds Mountains 50% of the time.
        if (percentChance(50)){
            addMountains(canvas, SCENE_HEIGHT, SCENE_WIDTH, SCENE_HEIGHT);
        }

        addGround(canvas, 400);

        // Adds Forest 60% of the time 
        if (percentChance(60)) {
            addForest(canvas, SCENE_HEIGHT, SCENE_WIDTH, SCENE_HEIGHT);
    ...
    private static List<GraphicsGroup> createFamily(int adultCount, int childCount) {
        ...
        ArrayList<GraphicsGroup> result = new ArrayList<>();

        for (int i = 0; i < adultCount; i++)
            result.add(createRandomEmoji(adultSize));
            
        for (int i = 0; i < childCount; i++)
            result.add(createRandomEmoji(childSize));
    ...
    private static void positionFamily(
        ...
        for(GraphicsGroup emoji : family){
            ...
            emoji.setPosition((accuX), (baselineY-height));
            accuX += spacing + width;
